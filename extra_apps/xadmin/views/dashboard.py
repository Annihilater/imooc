...
class UserWidgetAdmin(object):
    ...
    wizard_form_list = (
        ...
        (
            _(u"Widget Params"),
            {"callback": "get_widget_params_form", "convert": "convert_widget_params"},
        ),
    )
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        if db_field.name == "widget_type":
            ...
            return forms.ChoiceField(
                choices=[(w.widget_type, w.description) for w in widgets],
                widget=form_widget,
                label=_("Widget Type"),
            )
    ...
    def update_dashboard(self, obj):
        try:
            ...
            return
        pos = [
            [w for w in col.split(",") if w != str(obj.id)]
...
class HtmlWidget(BaseWidget):
    ...
    content = forms.CharField(
        label=_("Html Content"), widget=exwidgets.AdminTextareaWidget, required=False
    )
...
class ModelBaseWidget(BaseWidget):
    ...
    model = ModelChoiceField(
        label=_(u"Target Model"), widget=exwidgets.AdminSelectWidget
    )
    ...
    def model_admin_url(self, name, *args, **kwargs):
        return reverse(
            "%s:%s_%s_%s"
            % (self.admin_site.app_name, self.app_label, self.model_name, name),
            args=args,
            kwargs=kwargs,
        )
...
class ListWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    def context(self, context):
        ...
        context["result_headers"] = [
            c for c in list_view.result_headers().cells if c.field_name in base_fields
        ]
        context["results"] = [
            [
                o
        ...
        context["page_url"] = (
            self.model_admin_url("changelist") + "?" + urlencode(self.list_params)
        )
...
class AddFormWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    def setup(self):
        ...
        self.add_view = self.get_view_class(
            CreateAdminView, self.model, list_per_page=10
        )(req)
    ...
    def media(self):
        return (
            self.add_view.media
            + self.add_view.form_obj.media
            + self.vendor("xadmin.plugin.quick-form.js")
        )
...
class Dashboard(CommAdminView):
            ...
            if len(portal_pos):
                ...
                if portal_pos:
                    user_widgets = dict(
                        [
                            (uw.id, uw)
                            for uw in UserWidget.objects.filter(
    ...
    def get_context(self):
        new_context = {
            ...
            "columns": [
                ("col-sm-%d" % int(12 / len(self.widgets)), ws) for ws in self.widgets
            ],
            "has_add_widget_permission": self.has_model_perm(UserWidget, "add")
            and self.widget_customiz,
            ...
            if request.POST.get("_delete", None) != "on":
                ...
                try:
                    ...
                    try:
                        ...
                        pos = [
                            [w for w in col.split(",") if w != str(widget_id)]
                            for col in portal_pos.value.split("|")
                        ]
                        portal_pos.value = "|".join([",".join(col) for col in pos])
                        portal_pos.save()
    ...
    @filter_hook
    def get_media(self):
        media = super(Dashboard, self).get_media() + self.vendor(
...
class ModelDashboard(Dashboard, ModelAdminView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.template_response(
            self.get_template_list("views/model_dashboard.html"), self.get_context()
        )
