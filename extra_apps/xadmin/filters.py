...
class FieldFilterManager(object):
    ...
    def create(self, field, request, params, model, admin_view, field_path):
        for list_filter_class in self._field_list_filters:
            if not list_filter_class.test(
                field, request, params, model, admin_view, field_path
            ):
                continue
            return list_filter_class(
                field, request, params, model, admin_view, field_path=field_path
            )
...
class BooleanFieldListFilter(ListFieldFilter):
    ...
    def choices(self):
        for lookup, title in (("", _("All")), ("1", _("Yes")), ("0", _("No"))):
            yield {
                "selected": (
                    self.lookup_exact_val == lookup and not self.lookup_isnull_val
                ),
                "query_string": self.query_string(
                    {self.lookup_exact_name: lookup}, [self.lookup_isnull_name]
                ),
                "display": title,
            }
        if isinstance(self.field, models.NullBooleanField):
            yield {
                ...
                "query_string": self.query_string(
                    {self.lookup_isnull_name: "True"}, [self.lookup_exact_name]
                ),
                "display": _("Unknown"),
            }
...
class NumberFieldListFilter(FieldFilter):
    ...
    def test(cls, field, request, params, model, admin_view, field_path):
        return isinstance(
            field, (models.DecimalField, models.FloatField, models.IntegerField)
        )
...
class DateFieldListFilter(ListFieldFilter):
    ...
    def __init__(self, field, request, params, model, admin_view, field_path):
        ...
        self.links = (
            (_("Any date"), {}),
            (_("Has date"), {self.lookup_isnull_name: False}),
            (_("Has no date"), {self.lookup_isnull_name: "True"}),
            (
                _("Today"),
                {
...
class RelatedFieldSearchFilter(FieldFilter):
    ...
    def test(cls, field, request, params, model, admin_view, field_path):
        ...
        return related_modeladmin and getattr(
            related_modeladmin, "relfield_style", None
        ) in ("fk-ajax", "fk-select")
    ...
    def __init__(self, field, request, params, model, model_admin, field_path):
        ...
        self.label = (
            self.label_for_value(other_model, rel_name, self.lookup_exact_val)
            if self.lookup_exact_val
            else ""
        )
        self.choices = "?"
        if field.remote_field.limit_choices_to:
            for i in list(field.remote_field.limit_choices_to):
                self.choices += "&_p_%s=%s" % (
                    i,
...
class RelatedFieldListFilter(ListFieldFilter):
    ...
    def has_output(self):
        if (
            is_related_field(self.field)
            and self.field.field.null
            or hasattr(self.field, "remote_field")
            and self.field.null
        ):
            extra = 1
        else:
    ...
    def choices(self):
        ...
        for pk_val, val in self.lookup_choices:
            yield {
                ...
                "query_string": self.query_string(
                    {self.lookup_exact_name: pk_val}, [self.lookup_isnull_name]
                ),
                "display": val,
            }
        if (
...
class MultiSelectFieldListFilter(ListFieldFilter):
    ...
    def __init__(
        ...
        queryset = (
            ...
            .exclude(**{"%s__isnull" % field_path: True})
        ...
        if (
            ...
            and type(field_limit) == int
            and queryset.count() > field_limit
        ):
            queryset = queryset[:field_limit]

        self.lookup_choices = [
            str(it)
            for it in queryset.values_list(field_path, flat=True)
            if str(it).strip() != ""
        ]
    ...
    def choices(self):
        self.lookup_in_val = (
            (type(self.lookup_in_val) in (tuple, list))
            and self.lookup_in_val
            or list(self.lookup_in_val)
        )
        yield {
    ...
    def choices(self):
        ...
        for val in self.lookup_choices:
            ...
            yield {
                ...
                "query_string": self.query_string(
                    {self.lookup_exact_name: val}, [self.lookup_isnull_name]
                ),
                "display": val,
            }
        if include_none:
