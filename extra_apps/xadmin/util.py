...
def get_limit_choices_to_from_path(model, path):
    ...
    limit_choices_to = (
        fields
        and hasattr(fields[-1], "remote_field")
        and getattr(fields[-1].remote_field, "limit_choices_to", None)
    )
...
def is_related_field2(field):
    return (
        hasattr(field, "remote_field") and field.remote_field != None
    ) or is_related_field(field)
