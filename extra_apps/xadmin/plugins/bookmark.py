...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        bookmarks_queryset = Bookmark.objects.filter(
            content_type=content_type, url_name="xadmin:%s_%s_changelist" % model_info
            ...
            bookmarks.append(
                {
                    ...
                    "edit_url": reverse(
                        "xadmin:%s_%s_%s"
                        % (bk_model_info[0], bk_model_info[1], change_or_detail),
                        args=(bk.id,),
                    ),
                }
            )
        ...
        post_url = reverse(
            "xadmin:%s_%s_bookmark" % model_info, current_app=self.admin_site.name
        )
        ...
        new_context = {
            ...
            "has_add_permission_bookmark": self.admin_view.request.user.has_perm(
                "xadmin.add_bookmark"
            ),
            "has_change_permission_bookmark": self.admin_view.request.user.has_perm(
                "xadmin.change_bookmark"
            ),
        }
...
class BookmarkWidget(PartialBaseWidget):
    ...
    def setup(self):
        ...
        self.list_view = self.get_view_class(
            ListAdminView, model, list_per_page=10, list_editable=[]
        )(req)
