...
class RecoverListView(BaseReversionView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return TemplateResponse(
            ...
            or self.get_template_list("views/recover_list.html"),
            context,
        )
...
class RevisionListView(BaseReversionView):
    ...
    def get_response(self):
        ...
        return TemplateResponse(
            ...
            or self.get_template_list("views/model_history.html"),
            context,
        )
        ...
        for field_name, pks in obj_version.m2m_data.items():
            ...
            if f.rel and isinstance(f.rel, models.ManyToManyRel):
                setattr(
                    obj,
                    f.name,
                    f.rel.to._default_manager.get_query_set().filter(pk__in=pks).all(),
                )
        ...
        context.update(
            {
                ...
                "version_a": version_a,
                "version_b": version_b,
                "revision_a_url": self.model_admin_url(
                    "revision", quote(version_a.object_id), version_a.id
                ),
                "revision_b_url": self.model_admin_url(
                    "revision", quote(version_b.object_id), version_b.id
                ),
                "diffs": diffs,
            }
        )
    ...
    @filter_hook
    def get_media(self):
        return super(RevisionListView, self).get_media() + self.vendor(
            "xadmin.plugin.revision.js", "xadmin.form.css"
        )
...
class BaseRevisionView(ModelFormAdminView):
    ...
    @filter_hook
    def get_media(self):
        return super(BaseRevisionView, self).get_media() + self.vendor(
...
class RevisionView(BaseRevisionView):
    ...
    def get_form_helper(self):
        ...
        for f in self.opts.fields:
            ...
            if fvalue is None and vvalue == "":
                vvalue = None
            if is_related_field2(f):
                vvalue = version_data.get(
                    f.name + "_" + f.rel.get_related_field().name, None
                )
    ...
    def get_response(self):
        ...
        return TemplateResponse(
            self.request,
            form_template or self.get_template_list("views/revision_form.html"),
            context,
        )
    ...
    def post_response(self):
        self.message_user(
            _(
                'The %(model)s "%(name)s" was reverted successfully. You may edit it again below.'
            )
            % {
                "model": force_text(self.opts.verbose_name),
                "name": smart_text(self.new_obj),
            },
            "success",
        )
...
class RecoverView(BaseRevisionView):
    ...
    def get_response(self):
        ...
        return TemplateResponse(
            self.request,
            form_template or self.get_template_list("views/recover_form.html"),
            context,
        )
...
class InlineRevisionPlugin(BaseAdminPlugin):
    ...
    def _hack_inline_formset_initial(self, revision_view, formset):
        ...
        for related_version in related_versions.values():
            ...
            pk_name = (
                ContentType.objects.get_for_id(related_version.content_type_id)
                .model_class()
                ._meta.pk.name
            )
        ...
        if self.request.method == "GET" and formset.helper and formset.helper.layout:
            ...
            fake_admin_class = type(
                str("%s%sFakeAdmin" % (self.opts.app_label, self.opts.model_name)),
                (object,),
                {"model": self.model},
            )
            for form in formset.forms:
