...
class CommentsAdmin(object):
    form_layout = (
        Main(
            Fieldset(
                None, "content_type", "object_pk", "site", css_class="unsort no_title"
            ),
            Fieldset(
                "Content", "user", "user_name", "user_email", "user_url", "comment"
            ),
        ),
        Side(
            Fieldset(
                _("Metadata"), "submit_date", "ip_address", "is_public", "is_removed"
            )
        ),
    )
    ...
    def flag_comments(self, request, queryset):
        self._bulk_flag(
            queryset, perform_flag, lambda n: ungettext("flagged", "flagged", n)
        )
    ...
    def approve_comments(self, request, queryset):
        self._bulk_flag(
            queryset, perform_approve, lambda n: ungettext("approved", "approved", n)
        )
    ...
    def remove_comments(self, request, queryset):
        self._bulk_flag(
            queryset, perform_delete, lambda n: ungettext("removed", "removed", n)
        )
    ...
    def _bulk_flag(self, queryset, action, done_message):
        ...
        self.message_user(
            msg % {"count": n_comments, "action": done_message(n_comments)}, "success"
        )
