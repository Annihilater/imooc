...
class DetailsPlugin(BaseAdminPlugin):
    ...
    def result_item(self, item, obj, field_name, row):
        if self.show_all_rel_details or (field_name in self.show_detail_fields):
            rel_obj = None
            if hasattr(item.field, "remote_field") and isinstance(
                item.field.remote_field, models.ManyToOneRel
            ):
                ...
                if rel_obj.__class__ in site._registry:
                    try:
                        ...
                        has_view_perm = model_admin(
                            self.admin_view.request
                        ).has_view_permission(rel_obj)
                        has_change_perm = model_admin(
                            self.admin_view.request
                        ).has_change_permission(rel_obj)
                    except:
