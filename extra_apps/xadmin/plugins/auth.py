        ...
        if self.org_obj:
            self.form_layout = (
                Main(
                    ...
                    Fieldset(
                        _("Personal info"), Row("first_name", "last_name"), "email"
                    ),
...
class ModelPermissionPlugin(BaseAdminPlugin):
    ...
    def get_list_display(self, list_display):
        if (
            ...
            and self.user_owned_objects_field in list_display
        ):
...
class AccountMenuPlugin(BaseAdminPlugin):
    def block_top_account_menu(self, context, nodes):
        return '<li><a href="%s"><i class="fa fa-key"></i> %s</a></li>' % (
            self.get_admin_url("account_password"),
            _("Change Password"),
        )
...
class ChangePasswordView(ModelAdminView):
    ...
    def get_media(self):
        ...
        media = (
            media
            + self.vendor("xadmin.form.css", "xadmin.page.form.js")
            + self.form.media
        )
        return media
