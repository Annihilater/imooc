...
class RelateMenuPlugin(BaseAdminPlugin):
    ...
    def _get_all_related_objects(
        self, local_only=False, include_hidden=False, include_proxy_eq=False
    ):
        """
    ...
    def related_link(self, instance):
        ...
        return (
            '<div class="dropdown related_menu pull-right"><a title="%s" class="relate_menu dropdown-toggle" data-toggle="dropdown"><i class="icon fa fa-list"></i></a>%s</div>'
...
class BaseRelateDisplayPlugin(BaseAdminPlugin):
    ...
    def _get_input(self):
        return (
            '<input type="hidden" name="%s" value="%s" />' % self._get_relate_params()
        )
