...
AGGREGATE_METHODS = {"min": Min, "max": Max, "avg": Avg, "sum": Sum, "count": Count}
AGGREGATE_TITLE = {
    "min": _("Min"),
    "max": _("Max"),
    "avg": _("Avg"),
...
class AggregationPlugin(BaseAdminPlugin):
    ...
    def _get_aggregate_row(self):
        ...
        obj = queryset.aggregate(
            *[
                AGGREGATE_METHODS[method](field_name)
                for field_name, method in self.aggregate_fields.items()
                if method in AGGREGATE_METHODS
            ]
        )
        ...
        row.cells = [
            self._get_field_aggregate(field_name, obj, row)
            for field_name in self.admin_view.list_display
        ]
    ...
    def get_media(self, media):
        return media + Media(
            css={"screen": [self.static("xadmin/css/xadmin.plugin.aggregation.css")]}
        )
